...
7. 图片文件的实际读取根目录固定是 `layer_input/<file_id>/`，不是当前层自己的目录
...
- 第一层给出的 `images` 仍然只是相对路径列表
- 这些相对路径的实际根目录固定是 `layer_input/aaa/`
- 第二层读取图片时，必须从 `layer_input/<file_id>/` 下解析
- 不能把图片路径错误地解析到 `layer_problem/<file_id>/`
- 也不能解析到 `layer_cot/<file_id>/`
...
- `images` 保存的是相对于 `layer_input/<file_id>/` 的路径
- 例如 `images = ["test/img1.png"]`
- 则实际图片文件路径应解析为 `layer_input/<file_id>/test/img1.png`
...
- 第二层不重写 `images` 的相对路径语义
- 第二层只延续第一层已经给出的 `images`
- 真正读取图片时，统一以 `layer_input/<file_id>/` 作为根目录
...
- `images` 仍是相对路径列表
- `image_root` 固定指向 `layer_input/<file_id>/`
- 若生成器需要读取图片，应使用 `image_root + images[i]` 来解析实际文件
...
- 若需要读取图片，也必须从 `layer_input/<file_id>/` 解析 `images`
