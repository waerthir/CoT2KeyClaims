...
from config import OpenAICompatibleBackendConfig, PROBLEMS2COT_CONFIG, Problems2CotConfig
...
def _require_bool(value: Any, label: str) -> bool:
    if not isinstance(value, bool):
        raise ValueError(f"Expected {label} to be a boolean, got {type(value).__name__}.")
    return value
...
class ProblemContext:
    ...
    def image_root(self) -> Path:
        return self.work_unit.image_root
...
class UrllibJsonHttpTransport:
    def post_json(
        ...
        *,
        ...
        return _require_object(json.loads(body), "OpenAI-compatible JSON response")
...
class OpenAICompatibleChatClient:
        ...
        *,
        ...
        payload: dict[str, Any] = {
            ...
            "messages": [
                ...
                {
                    "role": "user",
                    "content": self._build_user_content(user_prompt=user_prompt, image_paths=image_paths),
                },
            ],
        ...
        return self._parse_json_object(self._extract_message_text(response))

    def _validate_api_key(self) -> None:
        api_key = self.config.api_key.strip()
        if not api_key:
            raise RuntimeError(f"{self.config.name} api_key is empty.")
        if "fake" in api_key.lower():
            raise RuntimeError(
                f"{self.config.name} api_key is still a placeholder. Replace it in config.py before calling the API."
            )

    def _build_request_url(self) -> str:
        return f"{self.config.base_url.rstrip('/')}/chat/completions"

    def _build_user_content(self, *, user_prompt: str, image_paths: list[Path]) -> list[dict[str, Any]]:
        content: list[dict[str, Any]] = [{"type": "text", "text": user_prompt}]
        for image_path in image_paths:
            content.append(
                {
                    "type": "image_url",
                    "image_url": {"url": self._read_image_as_data_url(image_path)},
                }
            )
        return content

    def _read_image_as_data_url(self, path: Path) -> str:
        if not path.is_file():
    ...
    def _extract_message_text(self, response: dict[str, Any]) -> str:
        ...
        if not isinstance(choices, list) or not choices:
            raise ValueError("OpenAI-compatible response is missing choices[0].")
        choice = _require_object(choices[0], "choices[0]")
        message = _require_object(choice.get("message"), "choices[0].message")
        ...
        if isinstance(content, list):
            ...
            for index, part in enumerate(content):
                block = _require_object(part, f"choices[0].message.content[{index}]")
                if block.get("type") != "text":
                    continue
                text_parts.append(_require_str(block.get("text"), f"choices[0].message.content[{index}].text"))
            if text_parts:
    ...
    def _parse_json_object(self, text: str) -> dict[str, Any]:
        ...
        for index, character in enumerate(text):
            ...
            if isinstance(value, dict):
                return value
        raise ValueError(f"Model output did not contain a JSON object: {text!r}")


class OpenAICompatibleCoTGenerationBackend:
    def __init__(
        self,
        config: Problems2CotConfig,
        ...
        self.config = config
        self.client = client or OpenAICompatibleChatClient(config.cot_generation_backend)
    ...
    def generate(
        ...
        *,
        ...
        response = self.client.create_json_response(
            system_prompt=self.config.cot_generation_system_prompt,
            user_prompt=(
                "Solve the problem and return JSON only.\n"
                'Required schema: {"cot": "<string>", "generated_answer": "<string>"}\n'
                f"Problem context:\n{json.dumps(request_payload, ensure_ascii=False, indent=2)}"
            ),
            image_paths=image_paths,
        )
        return GeneratedCoT(
            cot=_require_str(response.get("cot"), "response.cot"),
...
class OpenAICompatibleDuplicateCheckBackend:
    def __init__(
        self,
        config: Problems2CotConfig,
        client: OpenAICompatibleChatClient | None = None,
        ...
        self.config = config
        self.client = client or OpenAICompatibleChatClient(config.duplicate_check_backend)
    ...
    def is_duplicate(
        ...
        *,
        ...
        response = self.client.create_json_response(
            system_prompt=self.config.duplicate_check_system_prompt,
            user_prompt=(
                "Decide whether the candidate solution is materially the same method as any existing "
                "complete solution. Return JSON only.\n"
                'Required schema: {"is_duplicate": true, "reason": "<string>"}\n'
                f"Comparison payload:\n{json.dumps(comparison_payload, ensure_ascii=False, indent=2)}"
            ),
            image_paths=[],
        )
        return _require_bool(response.get("is_duplicate"), "response.is_duplicate")


class OpenAICompatibleGeminiReviewBackend:
    def __init__(
        self,
        config: Problems2CotConfig,
        client: OpenAICompatibleChatClient | None = None,
        ...
        self.config = config
        self.client = client or OpenAICompatibleChatClient(config.gemini_review_backend)
    ...
    def review(
        ...
        *,
        ...
        response = self.client.create_json_response(
            system_prompt=self.config.gemini_review_system_prompt,
            user_prompt=(
                "Check the fragment against the problem, standard answer, and attached images. Return "
                "JSON only.\n"
                'Required schema: {"passed": true, "cot": "<string>", "generated_answer": "<string>", '
                '"reason": "<string>"}\n'
        ...
        return GeminiReviewResult(
            passed=_require_bool(response.get("passed"), "response.passed"),
            cot=None if cot is None else _require_str(cot, "response.cot"),
            generated_answer=(
                None
                if generated_answer is None
                else _require_str(generated_answer, "response.generated_answer")
            ),
        )
...
class ProblemPackageReader:
    ...
    def _build_work_unit(self, work_unit_name: str) -> WorkUnit:
        ...
        image_root = self.config.image_root / work_unit_name
        if not image_root.is_dir():
            raise FileNotFoundError(f"Input image root not found: {image_root}")
...
class ProblemImageLocator:
    def resolve_paths(self, problem_context: ProblemContext) -> list[Path]:
        return [problem_context.image_root / relative_path for relative_path in problem_context.images]
...
class CoTGenerator:
    ...
    def create(self, problem_context: ProblemContext, method_id: int) -> FragmentSnapshot:
        image_paths = self.image_locator.resolve_paths(problem_context)
        generated = self.backend.generate(
            ...
            image_root=problem_context.image_root,
            image_paths=image_paths,
...
class GeminiDetailChecker:
    ...
    def process(self, fragment: FragmentSnapshot, problem_context: ProblemContext) -> bool:
        payload = dict(fragment.payload)
...
class FragmentTaskExecutor:
    ...
    def _advance_fragment(
        ...
        if payload.get(self.config.gemini_checked_field) is None:
            if not self.gemini_checker.process(fragment, problem_context):
                report.fragments_cleaned += 1
                return
            fragment = self.store.read_fragment(fragment_path)
            payload = fragment.payload
...
class Problems2CotPipeline:
    def __init__(
        ...
        *,
        ...
        self.image_locator = ProblemImageLocator()
        ...
        self.cot_generator = CoTGenerator(
            ...
            self.store,
            self.image_locator,
            generation_backend or OpenAICompatibleCoTGenerationBackend(config),
        )
        self.answer_matcher = AnswerMatcher(config, self.store)
        self.duplicate_checker = DuplicateMethodChecker(
            config,
            self.store,
            duplicate_backend or OpenAICompatibleDuplicateCheckBackend(config),
        )
        self.gemini_checker = GeminiDetailChecker(
            config,
            self.store,
            self.image_locator,
            gemini_backend or OpenAICompatibleGeminiReviewBackend(config),
        )
